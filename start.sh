...
if ! command -v inotifywait &> /dev/null; then
    echo "Error: inotifywait is not installed. Install it with: sudo apt install inotify-tools" >&2
    exit 1
fi

# Kill http-server if it is running
pkill -f "http-server _build/html" 2>/dev/null || true
...
# Rebuild site after each file change, including debounce of 1 second.
